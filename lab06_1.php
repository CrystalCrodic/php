		...
		<form action="lab06_1.php" method="post" enctype="multipart/form-data">
			<table align="center">
				...
				<tr>
					<td>Thông Tin:</td>
					<td>
						<textarea name="thong_tin"></textarea>
					</td>
				</tr>
				<tr>
	...
	?>
		<div class="info">
			...
			if ($err != "") echo $err;
				...
				echo "Mật khẩu đã mã hoá SHA1: " . sha1($password1) . "<br/>";
				echo "Họ tên: " . ucwords($name) . "<br/>";

				// Loại bỏ các thẻ html
				$notice = strip_tags($thongtin);
				// Thay thế /n thành <br/>
				$notice = nl2br($notice);
				// Thêm / trước '
				$notice = addslashes($notice);
				// Loại Bỏ / trước các ký tự đặc biệt
				$notice = stripslashes($notice);
				echo "Thông Tin: " . $notice;
			}
			?>
		</div>
	<?php
